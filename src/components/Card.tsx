...
export default function Card({ item }: { item: Card }) {
  return (
    <article className="w-[282px] h-[341px] rounded-card border space-y-4 border-neutral-100 p-3 shrink-0">
        <div className="w-full h-[213px] rounded-card overflow-hidden">
      <img src={item.img} alt={"image de " + item.name} className="object-cover w-full h-full" />
    ...
    <button type="button" className="w-full border border-neutral-800 rounded-xl flex justify-center py-[10px]">Place a Bild</button>
    </article>
  );
}
